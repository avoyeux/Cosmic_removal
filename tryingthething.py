...
class Cosmicremoval_class:
    ...
    def time_interval(self, exposure, detector, filenames, files, images, positions, SPIOBSID):
        ...
        for loop in range(len(files)):
            ...
            mad, mode, chunks_masks = self.Chunks_func(nw_used_images, image_index)
            mads.append(mad)
            modes.append(mode)
            masks.append(chunks_masks)
    ...
    def Chunk_madmodemask(self, chunk, image_index):
        ...
        masks_filter = chunk[image_index] > self.coef * mad_array + mode_array
        masks[masks_filter] = True
        return mad_array, mode_array, masks  # these are all the values for each chunk

    def Chunks_func(self, images, image_index):
        ...
        chunks_mad, chunks_mode, chunks_mask = self.Chunk_madmodemask(chunks[0], image_index)
        for loop in range(1, len(chunks)):
            chunk = chunks[loop]
            chunk_mad, chunk_mode, chunk_mask = self.Chunk_madmodemask(chunk, image_index)
            ...
            chunks_mask = np.concatenate((chunks_mask, chunk_mask), axis=0)  # TODO: need to check if it axis 0 or 1
