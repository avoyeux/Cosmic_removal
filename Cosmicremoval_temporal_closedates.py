...
class Cosmicremoval_class:
    ...
    def __init__(self, processes=2, chunk_nb=4, coefficient=6, min_filenb=30, months_interval=8, min_files=12):
        ...
        # Miscellaneous
        self.ref_pixels = list(map(int, np.linspace(0, 1023, 5)))  # reference pixels for the init madmode calculations
    ...
    def Paths(self, exposure='none', detector='none'):
        ...
        """
        main_path = os.path.join(os.getcwd(), f'Temporal_coef{self.coef}_{self.months_interval}months_bins2000_final')
    ...
    def Multiprocess(self):
        ...
        if self.processes > 1:
            ...
            args = [(exposure,) for exposure in self.exposures[2:]]
    ...
    def Main(self, exposure):
        ...
        for detector in range(2):
            ...
            # TODO: should change the next line so that the code only searches and opens the files once for both detectors
    ...
    def time_interval(self, exposure, detector, filenames, files, images, positions, SPIOBSID):
        ...
        for loop in range(len(files)):
            ...
            mads.append(mad)
            modes.append(mode)
            masks.append(chunks_masks[index_n - len(delete1)])  # Mask of the dark for the studied ID
        ...
        # self.Plotting_init(paths, used_images, mad, mode, SPIOBSID)
    ...
    ################################################ PLOTTING functions ################################################
    def Plotting_init(self, paths, all_images, ref_mad, ref_mode, SPIOBSID):
        ...
        for r in self.ref_pixels:
            for c in self.ref_pixels:
                ...
                # REF HISTO plotting
                hist_name = f'histo_{SPIOBSID}_r{r}_c{c}.png'
