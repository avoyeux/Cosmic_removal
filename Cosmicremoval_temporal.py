...
class Cosmicremoval_class:
    ...
    def __init__(self, processes=2, chunk_nb=4, coefficient=6, min_filenb=40):
        # Inputs
        ...
        # Miscellaneous
        self.ref_pixels = list(map(int, np.linspace(0, 1023, 16)))  # reference pixels for the init madmode calculations
    ...
    def Paths(self, exposure='none', detector='none'):
        ...
        """
        main_path = os.path.join(os.getcwd(), f'Temporal_coef{self.coef}')
    ...
    def Images_all(self, exposure, detector):
        ...
        date_array = np.array(list(res['DATE-BEG']))
        ...
        for loop, file in enumerate(filenames):
            ...
            if hdul[detector].header['BLACKLEV'] == 1:  # For on-board bias subtraction images
                left_nb += 1
                continue
            OBS_DESC = hdul[detector].header['OBS_DESC'].lower()
            ...
            dsun_list.append(dist_SUN)
            temp_list.append(temp)
            date_list.append(date_array[loop])
            filenames_nw.append(file)
        # Changing to arrays
        images_all = np.array(images_all)
        dsun_array = np.array(dsun_list)
        temp_array = np.array(temp_list)
        date_array = np.array(date_list)
        filenames_nw = np.array(filenames_nw)

        if a != 0:
            print(f'\033[31mExp{exposure}_det{detector} -- Nb files with high temp: {a}\033[0m')
        if left_nb != 0:
            print(f'\033[31mExp{exposure}_det{detector} -- Nb files with bias subtraction: {left_nb}\033[0m')
        if weird_nb != 0:
            print(f'\033[31mExp{exposure}_det{detector} -- Nb of "weird" files: {weird_nb}\033[0m')
        print(f'Exp{exposure}_det{detector} -- Nb of "usable" files: {len(filenames_nw)}')
        if len(filenames_nw) == 0:
            print(f'\033[91m ERROR: NO "USABLE" ACQUISITIONS - STOPPING THE RUN\033[0m')
            sys.exit()

        return images_all, date_array, dsun_array, temp_array, filenames_nw
    ...
    def Main(self, exposure):
        ...
        for detector in range(2):
            ...
            images, date, dsun, temp, filenames = self.Images_all(exposure, detector)
            ...
            # Creation of the main n left list with the mad and modes
            ...
            chunks, chunks_mad, chunks_mode, chunks_mask = self.Chunks_func(images)
            print(f'Exp{exposure}_det{detector} -- Chunks finished.')
            ...
            for SPIOBSID, files in same_darks.items():
                ...
                self.Medianplotting(paths, files, data, masks, chunks_mode)
                # self.Stats_plotting(paths, files, data, nw_masks)

                # # Saving the stats in a csv file
                # data_pandas = self.Unique_datadict(exposure, detector, files, chunks_mad, chunks_mode, detections,
            ...
            print(f'Exp{exposure}_det{detector} -- Median plotting done.')
    ...
    def Chunks_func(self, images):
        ...
        chunks_mad, chunks_mode, chunks_mask = self.Chunk_madmodemask(combined_chunk)
        for loop in range(1, len(chunks)):
            chunk = chunks[loop]
            chunk_mad, chunk_mode, chunk_mask = self.Chunk_madmodemask(chunk)

            # Saving the data
            combined_chunk = np.concatenate((combined_chunk, chunk), axis=1)  # (imgnb*1024*1024 array)
        ...
        return combined_chunk, chunks_mad, chunks_mode, chunks_mask
    ...
    def Stats(self, data, masks, mode):
        ...
        # Creating a mode array that has the same shape than the images and masks
        nw_mode = np.zeros_like(data)
        nw_mode[:] = mode

        # Difference between the end result and the initial one
        nw_data[masks] = nw_mode[masks]
    ...
    ################################################ PLOTTING functions ################################################
    def Plotting_init(self, paths, all_images, ref_mad, ref_mode):
    ...
    def Medianplotting(self, paths, files, data, masks, mode):
        ...
        for loop, image in enumerate(data):
            ...
            # DARKS plotting
            plot_name = f'Dark_{date.year:04d}{date.month:02d}{date.day:02d}_H{date.hour:02d}_{date.minute:02d}_{SPIOBSID}.pdf'
            plt.imshow(image, interpolation='none')
            plt.title(f'Dark: {file}')
            plt.colorbar()
            plt.savefig(os.path.join(paths['Darks'], plot_name))
            plot_name = f'Dark_cont_{date.year:04d}{date.month:02d}{date.day:02d}_H{date.hour:02d}_{date.minute:02d}_{SPIOBSID}.pdf'
            for line in lines:
                plt.plot(line[1], line[0], color='r', linewidth=0.05)
            plt.savefig(os.path.join(paths['Darks'], plot_name))
            plt.close()
            ...
            # LOG DARKS plotting
            plot_name = f'Log_dark_{date.year:04d}{date.month:02d}{date.day:02d}_H{date.hour:02d}_{date.minute:02d}_{SPIOBSID}.pdf'
            ...
            cbar.update_ticks()
            plt.savefig(os.path.join(paths['Darks'], plot_name))
            plot_name = f'Log_darkcont_{date.year:04d}{date.month:02d}{date.day:02d}_H{date.hour:02d}_{date.minute:02d}_{SPIOBSID}.pdf'
            for line in lines:
                plt.plot(line[1], line[0], color='r', linewidth=0.05)
            plt.savefig(os.path.join(paths['Darks'], plot_name))
            plt.close()
            ...
            # FINAL plotting
            plot_name = f'Final_{date.year:04d}{date.month:02d}{date.day:02d}_H{date.hour:02d}_{date.minute:02d}_{SPIOBSID}.pdf'
            plt.imshow(img, interpolation='none')
            plt.title(f'Final: {file}')
            plt.colorbar()
            plt.savefig(os.path.join(paths['Results'], plot_name))
            plot_name = f'Final_cont_{date.year:04d}{date.month:02d}{date.day:02d}_H{date.hour:02d}_{date.minute:02d}_{SPIOBSID}.pdf'
            for line in lines:
                plt.plot(line[1], line[0], color='r', linewidth=0.05)
            plt.savefig(os.path.join(paths['Results'], plot_name))
            plt.close()
    ...
    def Bins(self, data):
        ...
        bins = int(len(data) * val_range / 300)
        if bins < 20:
            bins = 20
        return bins
...
if __name__ == '__main__':
    ...
    test = Cosmicremoval_class(processes=1, min_filenb=30)
    test.Multiprocess()
